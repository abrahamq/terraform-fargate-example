/** FROM: https://github.com/turnerlabs/terraform-ecs-fargate/blob/master/env/dev/autoscale-perf.tf
 * with an apache2.0 license.
...
resource "aws_appautoscaling_target" "app_scale_target" {
  ...
  resource_id        = "service/${aws_ecs_cluster.this.name}/${aws_ecs_service.data.name}"
  scalable_dimension = "ecs:service:DesiredCount"
  max_capacity       = var.ecs_autoscale_max_instances
  min_capacity       = var.ecs_autoscale_min_instances
}

resource "aws_cloudwatch_metric_alarm" "data_cpu_utilization_high" {
  alarm_name          = "data-CPU-Utilization-High-${var.ecs_as_cpu_high_threshold_per}"
  ...
  dimensions = {
    ...
    ServiceName = aws_ecs_service.data.name
  }
...
resource "aws_cloudwatch_metric_alarm" "cpu_utilization_low" {
  alarm_name          = "data-CPU-Utilization-Low-${var.ecs_as_cpu_low_threshold_per}"
